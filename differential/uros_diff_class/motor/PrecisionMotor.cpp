...
PrecisionMotor::PrecisionMotor(uint ena_pin, uint in1_pin, uint in2_pin,
                               ...
                               float kp, float ti, float td, float correction_factor)
    : Motor(ena_pin, in1_pin, in2_pin),
      ...
      setpoint_percentage(0.0f), control_output(0.0f), correction_factor(correction_factor) {
...
float PrecisionMotor::get_setpoint() const { 
    return setpoint_percentage*(MAX_RPM/100.0f); // Convert back to RPM
}
